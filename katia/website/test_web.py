...
def index():
    ...
    print(station_id)
    if station_id == None:
        prediction, station_name = run_model()
        predictions = prediction
        return render_template("index.html",predictions = predictions, stations = station_name)
    else:
        return render_template("index.html",predictions = predictions.loc[predictions['id']==int(station_id)], stations = station_name)
...
def run_model():
    ...
    return predict_df2.sort_values(by=['id','datetime']), station_name
